...
export default class ThemeMultiPush extends Command {
  ...
  async run(): Promise<void> {
    ...
    const failedPushes: { env: string; error: string }[] = [];
    ...
    const buildArgs = (env: string, resolvedPath: string) => {
      const args = ["theme", "push", "-e", env];
      if (resolvedPath !== ".") args.push("--path", resolvedPath);
    ...
    if (isDryRun) {
      ...
      for (let i = 0; i < matchingEnvs.length; i += batchSize) {
        ...
        const pushPromises = batchEnvs.map((env) => {
          ...
          const executeAsyncPush = (attempt: number): Promise<void> => {
      ...
      for (const env of matchingEnvs) {
        ...
        const executeSyncPush = (attempt: number): Promise<void> => {
          return new Promise((resolve) => {
            if (attempt > 1) {
